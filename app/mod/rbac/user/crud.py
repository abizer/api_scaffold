...
class CRUDKey(CRUDBase[KeyTable, KeyCreate, KeyUpdate]):
    async def get_for_user(self, user_id: uuid.UUID, db_session: AsyncSession | None = None) -> Optional[KeyTable]:
...
class CRUDUser(CRUDBase[UserTable, UserCreate, UserUpdate]):
    async def get_by_email(self, email: str, db_session: AsyncSession | None = None) -> Optional[UserTable]:
        db_session = db_session or self.db.session 
